...
export default function UserActionsMenu({ targetUserId, targetPetName, matchId, onBlocked, iconColor = '#374151', bg = '#F3F4F6' }) {
  ...
  async function blockUser() {
    if (!window.confirm(`¿Bloquear a ${targetPetName || 'este usuario'}? No podrán verse ni contactarse más.`)) return
    setSubmitting(true)
      ...
      {showMenu && !showReport && (
        <div className="absolute inset-0 bg-black/50 z-50 flex items-end" onClick={() => setShowMenu(false)}>
          <div className="bg-white rounded-t-3xl w-full p-4 flex flex-col gap-2" onClick={e => e.stopPropagation()}>
            ...
            >
              <Flag size={16} /> Reportar a {targetPetName || 'este usuario'}
            </button>
            <button
              onClick={blockUser}
              disabled={submitting}
            ...
            >
              <ShieldOff size={16} /> Bloquear a {targetPetName || 'este usuario'}
            </button>
            <button
        ...
        <div className="absolute inset-0 bg-black/50 z-50 flex items-end">
          <div className="bg-white rounded-t-3xl w-full p-4 flex flex-col gap-3 max-h-[80%] overflow-y-auto">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-gray-900 text-base">Reportar a {targetPetName || 'este usuario'}</h3>
            ...
            <div>
              <label className="text-xs font-medium text-gray-500 mb-1 block">Detalles (opcional)</label>
              <textarea
